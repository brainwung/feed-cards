...
电商比价/优惠类信息卡片组件，左图右文自适应布局，固定 139px 高度。适用于"什么值得买"、"慢慢买"等返利/比价社区的商品推送场景。
...
- **两种布局** (v1.1)：
  - 左图右文（信息流单条，固定 139px 高度）
  - 上图下文 `.deal-card--vertical`（两列瀑布流，高度随内容）
- **自适应布局**：图片固定 115×115（水平版）/ 100%(上图下文)
- **价格特殊字体**：DIN 工业风 + 整数突出（16px）+ 小数/符号 13px，baseline 对齐
- **图文环绕标题**：徽标 `<img>` float left，2 行硬截断
