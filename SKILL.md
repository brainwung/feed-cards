---
name: feed-cards
description: App 信息流卡片设计系统 — 包含多种卡片类型，共享同一套视觉语言（375 视口设计、相同字体/标签/圆角/间距规则）。① 商品卡片 product-card（CSS 类名 .deal-card）：电商比价/优惠商品卡，含商品图/标题/标签/价格区/footer，支持左图右文(139px)和上图下文(瀑布流)两种布局，含售罄态。② 原创卡片 article-card（CSS 类名 .deal-card.deal-card--article）：UGC 内容卡，无价格区，footer 左侧为头像+用户名+认证角标，右侧 thumb-up 替代值图标。当用户提到"商品卡片"、"product card"、"好价卡片"、"deal card"、"原创卡片"、"article card"、"UGC 卡片"时使用。后续会扩展更多卡片类型，所有卡片共用本 skill 的标签系统、字体、颜色规则。
---

# Feed Cards · 信息流卡片设计系统 v2.0
...
## 19. Reference 页专属 UI（不属于 skill 规范）

`reference.html` 有两个仅用于演示/调试的 UI 元素，**业务接入时请删除**：

### 19.1 主题切换按钮 `.theme-toggle`

右上角固定按钮，点击切换 `data-theme="dark"` / `data-theme="light"`，便于实时预览深浅色。

### 19.2 顶部 Tabs（`.tabs`）

把横版/竖版示例分到两个 `.tab-panel` 下，仅用于 reference 页的示例分类。生产环境的卡片列表不需要 tabs（除非业务确实有此需求，那也属于业务页面层级，不属于 skill 范围）。

**接入清理步骤**：

1. 删除 `.theme-toggle` 按钮 HTML、对应 CSS、对应 onclick JS
2. 删除 `.tabs` HTML、对应 CSS、对应 tab 切换 JS
