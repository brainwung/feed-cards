...
| `community-card / repost` | 社区卡片转载模式 | `.deal-card.deal-card--community.deal-card--community-repost` | ✓ v2.5 新增 | 单一社区频道推荐转载，左图右文外部内容预览 |
| _（未来扩展）_ | — | — | — | — |
...
| 转载模式 | `.deal-card.deal-card--community.deal-card--community-repost` | 头部去掉头像，显示"分享自：" + 用户名，二者无额外间距，label 为"推荐"，无外层标题，摘要后接左图右文转载预览 |
...
社区转载模式用于推荐外部社区内容。外层仍使用 `.deal-card.deal-card--community`，但头部去掉头像，头像位置改为"分享自："文字，色值 `#333333`（深色跟随主文字色），与用户名之间不加额外间距，仅使用标点自带间距；状态文案从"原创"改为"推荐"。不显示 `.deal-card__community-title`，只保留 `.deal-card__community-summary`。

转载预览替代普通图片区域，结构类似横版原创卡片，但不使用标签行：

| 元素 | 规则 |
|---|---|
| 预览容器 | `.deal-card__community-repost-card`，高 110px，圆角 3px，背景使用 `var(--c-bg-product-tag)`：浅色 `#F5F5F5`，深色 `#353535` |
| 左图 | `.deal-card__community-repost-image`，110×110px，1:1 |
| 右侧内容 | `.deal-card__community-repost-info`，padding `12px 9px 9px`，纵向 space-between |
| 转载标题 | `.deal-card__community-repost-title`，距预览卡顶部 12px，15px / 1.5 / 700，最多 2 行，超出显示省略号 |
| 底部作者 | `.deal-card__community-repost-author`，左下角，头像 15px + 名字 12px / 1.2 / `#999999` |
| 来源 | `.deal-card__community-repost-source`，右下角，12px / 1.2，文案固定为"来自小红书" |
